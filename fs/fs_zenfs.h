...
class ZenFS : public FileSystemWrapper {
  ...
  void LockFilesMutex() { files_mtx_.lock(); }
  void UnlockFilesMutex() { files_mtx_.unlock(); }
  ...
  IOStatus SyncFileExtentsNoLock(ZoneFile* zoneFile,
                                 std::vector<ZoneExtent*> new_extents);
 ...
 public:
  ...
  IOStatus CompactZone(uint64_t zone_start,
...
class Path {
 public:
  ...
  std::string Normalize() {
    ...
    bool has_terminator = src_path_.back() == '/';
    if (!has_terminator) {
      filename_ = segs.back();
    }
